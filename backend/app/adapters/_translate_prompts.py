...
# 输出格式（极其重要）
- user 每次给一个 JSON 对象：{{"items": ["原文1", "原文2", ...]}}，含若干待译句子。
- 你必须返回严格的 JSON 对象：{{"items": [{{"dst": "<译文1>"}}, {{"dst": "<译文2>"}}]}}
- items 数量必须与输入完全一致，顺序一一对应，逐句翻译。
- dst 字段只放译文本身，不要解释、引号、编号、markdown。
...
- The user sends a JSON object: {{"items": ["sentence1", "sentence2", ...]}} with several sentences to translate.
- You MUST reply with a strict JSON object: {{"items": [{{"dst": "<translation1>"}}, {{"dst": "<translation2>"}}]}}
- The number of items must exactly match the input, in the same order, one translation per sentence.
- The dst field contains only the translation: no quotes, labels, prefixes, numbering or markdown.
